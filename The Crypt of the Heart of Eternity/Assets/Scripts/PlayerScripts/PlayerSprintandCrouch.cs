using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class PlayerSprintandCrouch : MonoBehaviour
{
    private PlayerMovement playerMovement;

    public float sprint_Speed = 10f;

    public float move_Speed = 5f;

    public float crouch_Speed = 2f;

    private Transform Look_Root;
    private float stand_Height = 1.6f;
    private float crouch_Height = 1f;

    private bool is_Crouch;

    private PlayerFootSteps player_Footsteps;

    private float sprint_Volume = 1f;
    private float crouch_Volume = 0.1f;
    private float walk_Volume_Min = 0.2f, walk_Volume_Max = 0.6f;

    private float walk_Step_Distance = 0.4f;
    private float sprint_Step_Distance = 0.25f;
    private float crouch_Step_Distance = 0.5f;

    private float boost = 1;

    private PlayerStates player_states;

    private float sprint_Value = 100f;
    public float sprint_threshold = 10f;

    void Awake()
    {
        playerMovement = GetComponent<PlayerMovement>();

        Look_Root = transform.GetChild(0);

        player_Footsteps = GetComponentInChildren<PlayerFootSteps>();

        player_states = GetComponent<PlayerStates>();
    }

    void Start()
    {
        player_Footsteps.volume_Min = walk_Volume_Min;
        player_Footsteps.volume_Max = walk_Volume_Max;
        player_Footsteps.step_Distence = walk_Step_Distance;
    }

    // Update is called once per frame
    void Update()
    {
        Sprint();
        Crouch();

        if (playerMovement.boosted)
        {
            boost = 2;
        }
        else
        {
            boost = 1;
        }
    }

    void Sprint()
    {
        if (sprint_Value > 0)
        {
            if (Input.GetKeyDown(KeyCode.LeftShift) && !is_Crouch)
            {
                playerMovement.speed = sprint_Speed*boost;

                player_Footsteps.step_Distence = sprint_Step_Distance;
                player_Footsteps.volume_Min = sprint_Volume;
                player_Footsteps.volume_Max = sprint_Volume;

            }
        }
        if(Input.GetKeyUp(KeyCode.LeftShift) && !is_Crouch)
        {
            playerMovement.speed = move_Speed * boost;

            player_Footsteps.step_Distence = walk_Step_Distance;
            player_Footsteps.volume_Min = walk_Volume_Min;
            player_Footsteps.volume_Max = walk_Volume_Max;
            


        }

        if (Input.GetKey(KeyCode.LeftShift) && !is_Crouch)
        {
            sprint_Value -= sprint_threshold * Time.deltaTime;

            if (sprint_Value <= 0f)
            {
                sprint_Value = 0f;

                playerMovement.speed = move_Speed * boost;
                player_Footsteps.step_Distence = walk_Step_Distance;
                player_Footsteps.volume_Min = walk_Volume_Min;
                player_Footsteps.volume_Max = walk_Volume_Max;

               
            }
            player_states.Display_StaminaStats(sprint_Value);
        }
        else
        {
            if (sprint_Value != 100)
            {
                sprint_Value += (sprint_threshold / 2f) * Time.deltaTime;
                player_states.Display_StaminaStats(sprint_Value);

                if (sprint_Value > 100f)
                {
                    sprint_Value = 100f;
                }
            }
        }
        
    }

    void Crouch()
    {
        if (Input.GetKeyDown(KeyCode.LeftControl))
        {
            //if crouching stand up
            if (is_Crouch)
            {
                Look_Root.localPosition = new Vector3(0f, stand_Height, 0f);
                playerMovement.speed = move_Speed * boost;

                player_Footsteps.step_Distence = walk_Step_Distance;
                player_Footsteps.volume_Min = walk_Volume_Min;
                player_Footsteps.volume_Max = walk_Volume_Max;

                is_Crouch = false;
            }
            //if not crouching crouch
            else
            {
                Look_Root.localPosition = new Vector3(0f, crouch_Height, 0f);
                playerMovement.speed = crouch_Speed * boost;


                player_Footsteps.step_Distence = crouch_Step_Distance;
                player_Footsteps.volume_Min = crouch_Volume;
                player_Footsteps.volume_Max = crouch_Volume;

                is_Crouch = true;
            }

        }//if you can Crouch
    }
}
